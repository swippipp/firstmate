...
near-miss claude-zai spelling stays flagged^{"rules":[{"when":"anything","use":{"harness":"claude-zay"}}]}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - unverified harness: claude-zay
claude-zai GLM profile with model alias threads effort silently^{"rules":[{"when":"validation chain","use":{"harness":"claude-zai","model":"opus","effort":"xhigh"}}]}^empty^
