...
The identity claim goes deeper than the family fold: `claude-zai` launches the same claude binary behind the numbered `claudeN` wrapper, merely pointed at z.ai's endpoint, which is why every busy-state, composer-shape, interrupt, exit, and lifecycle fact documented under `claude` carries over verbatim - `bin/fm-busy-lib.sh` and `bin/fm-composer-lib.sh` deliberately grow no claude-zai branch.
...
| Model flag | Threads like `claude` (captain 26.08., measured). The wrapper maps the aliases onto plan models, chosen by the captain after the measured O-0119 benchmark - see the Verification trail below: `sonnet`/`haiku`/small-fast/subagents -> `glm-5.3-flash`, `opus` -> `glm-5.3-flash[1m]`, `fable` and the no-model default -> `glm-5.3[1m]` (firstmate seat only). glm-5.3-flash is the unreleased Ox-Alpha heir (multimodal, image input verified); fallback if the stealth slug dies: sonnet/haiku -> glm-4.7, opus -> glm-5.3[1m]. The `[1m]` suffix is a CLIENT-side convention (the API rejects the literal slug; Claude Code uses it only to size its own context). `glm-5.2` no longer exists as a served tier (the server answers it AS glm-5.3, echo measured); `glm-5-turbo` hangs on direct probes (2x timeout 26.08.) - use neither. |
...
**Verification trail (2026-08-26, all under orders O-0116/O-0117/O-0119).**
Productive use came first: the firstmate primary itself ran on `claude2 --zai` from ~15:5x until the standing 19:00 hand-back to Anthropic account 2 while executing O-0116, with the totmann override adapted for the route.
The launch mechanics behind the profile shipped in the repo itself: e680ed6 landed the `claude1 --zai` template plus model and effort threading through `bin/fm-spawn.sh`'s shared family cases (`claude-zai` rides the same `--model` case as `claude` and the same `claude|claude-ox|claude-zai` `--effort` case), and fa0e189 remapped the aliases onto the Flash tiers.
The O-0119 benchmark that decided the Flash mapping measured build tasks 8/8 vs 8/8 and bug hunts 7/7 vs 7/7 with identical diffs between glm-5.3-flash and glm-5.3, at roughly one third the price.
The route's smoke instance is real dispatch traffic, not a lab run: this record's own verifying worker ran supervised through the same profile - its task meta records `harness=claude-zai`, `account=konto-1`, `model=sonnet`, `effort=high`, i.e. exactly the default rung of the production `config/crew-dispatch.json`.
