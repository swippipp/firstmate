...
crew_dispatch_validate() {
  ...
  err=$(jq -r '
    def verified($h): ["claude","claude-zai","codex","opencode","pi","pi-signed","grok","kimi","cursor","muse"] | index($h);
    def effort_ok($h; $e):
      if $e == null then true
      elif ($e | type) != "string" then false
      elif $h == "claude" or $h == "claude-zai" then (["low","medium","high","xhigh","max"] | index($e))
